...
def test_add_and_recent(tmp_path):
    ...
    h.add("raw one", "formatted one", "Mail", "Email")
    h.add("raw two", "formatted two", "Slack", "Message")
    rows = h.recent(10)
    assert len(rows) == 2
    assert rows[0]["formatted"] == "formatted two"  # newest first
    assert rows[0]["app"] == "Slack" and rows[0]["profile"] == "Message"
...
def test_cjk_roundtrip(tmp_path):
    h = History(str(tmp_path / "h.sqlite3"))
    h.add("你好呀 hello", "你好呀，hello!", "Notes", "Clean")
    row = h.recent(1)[0]
