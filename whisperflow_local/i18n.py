...
S = {
    ...
    "status_ai_copied": ("AI result copied to clipboard:", "AI 結果已複製到剪貼簿："),
    ...
    "menu_quit": ("Quit", "結束"),
    "profile_Raw": ("Raw", "原文"),
    "profile_Clean": ("Clean", "清理"),
    "profile_Email": ("Email", "電郵"),
    "profile_Message": ("Message", "訊息"),
    "profile_Notes": ("Notes", "筆記"),
    "lang_auto": ("Auto", "自動"),
    "lang_yue": ("Cantonese 廣東話", "廣東話"),
    "lang_en": ("English", "英文"),
    "lang_mixed": ("Mixed 中英夾雜", "中英夾雜"),
    "cmd_Formalize": ("Formalize", "改正式啲"),
    "cmd_Summarize": ("Summarize", "摘要"),
    "cmd_Translate to English": ("Translate to English", "譯做英文"),
    "cmd_Translate to Cantonese": ("Translate to Cantonese", "譯做廣東話"),
    "dlg_no_dictation": (
        "No dictation yet — dictate something first.",
        "仲未有口述內容 — 請先口述一次。",
    ),
    "dlg_clipboard_empty": ("Clipboard is empty.", "剪貼簿係空嘅。"),
    "dlg_which_command": ("Which command? ", "邊個指令？"),
    ...
    "dlg_rules_prompt": (
        'Per-app rules as JSON {"AppNameSubstring": "Profile"} (profiles: {names}):',
        '逐 App 規則（JSON，格式 {"App名稱關鍵字": "模式"}，模式用英文內部名：{names}）：',
    ),
